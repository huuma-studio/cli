...
export interface ManagedConfig {
   ...
   * fails transiently (from `--retries`; parser default 2, `0` disables).
   * The runner bounds retries with the shared backoff constants and never
   * starts one inside the terminal reserve (ADR 0010). */
  retries: number;
